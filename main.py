...
def main(_):
    ...
    setup_wandb(project='ogcrl', group=FLAGS.run_group, name=exp_name, entity='stitching-models', mode=FLAGS.wandb_mode)
    ...
    config = FLAGS.agent
    if config.get('n_step', None):
        config['frame_stack'] = config['n_step']
    ...
    if config.get('n_step', None):
        obs_shape = example_batch['observations'].shape[1:-1]
        example_obs = example_batch['observations'].reshape(1, *obs_shape, config['n_step'], -1)[..., 0, :]
    else:
    ...
    for i in tqdm.tqdm(range(1, FLAGS.train_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        if i == 1 or i % FLAGS.eval_interval == 0:
            ...
            if 'evaluation/overall_success' in eval_metrics:
                successes.append(eval_metrics['evaluation/overall_success'])
    ...
    # calculate average over last 3 overall_sucess, 
    # add to wandb summary
    wandb.run.summary['evaluation/overall_success_3'] = np.mean(successes[-3:])
    wandb.run.summary['evaluation/len_3'] = len(successes[-3:])
    train_logger.close()
    eval_logger.close()
