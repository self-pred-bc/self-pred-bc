...
class GCDataset:
    ...
    def sample(self, batch_size: int, idxs=None, evaluation=False):
        ...
        if 'pred_p_trajgoal' in self.config:
            pred_goal_idxs = self.sample_goals(
                idxs,
                self.config['pred_p_curgoal'],
                self.config['pred_p_trajgoal'],
                self.config['pred_p_randomgoal'],
                self.config['pred_geom_sample'],
            )
            batch['pred_goals'] = self.get_observations(pred_goal_idxs)
    ...
    def get_qpos(self, idxs):
        if self.config['frame_stack'] is None or self.preprocess_frame_stack:
            return jax.tree_util.tree_map(lambda arr: arr[idxs], self.dataset['qpos'])
        else:
            return self.get_stacked_qpos(idxs)

    def get_stacked_qpos(self, idxs):
        initial_state_idxs = self.initial_locs[np.searchsorted(self.initial_locs, idxs, side='right') - 1]
        rets = []
        for i in reversed(range(self.config['frame_stack'])):
            cur_idxs = np.maximum(idxs - i, initial_state_idxs)
            rets.append(jax.tree_util.tree_map(lambda arr: arr[cur_idxs], self.dataset['qpos']))
        return jax.tree_util.tree_map(lambda *args: np.concatenate(args, axis=-1), *rets)
