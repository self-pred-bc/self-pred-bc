...
class GCBilinearValue(nn.Module):
    ...
    normalize_psi: bool = False
    compute_v: bool = True
    action_forward: bool = False
    ...
    def __call__(self, observations, goals, actions=None, info=False):
        ...
        if self.action_forward and actions is not None:
            phi_inputs = jnp.concatenate([observations, actions], axis=-1)
        else:
            phi_inputs = observations

        if self.action_forward and actions is None:
            phi = self.psi(phi_inputs)
        else:
            phi = self.phi(phi_inputs)
        psi = self.psi(goals)
        if self.normalize_psi:
            psi = psi / jnp.linalg.norm(psi, axis=-1, keepdims=True) * jnp.sqrt(self.latent_dim)
...
class GCPredValue(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.psi_b = None
        if self.ensemble:
            self.phi = ensemblize(mlp_module, self.ensemble_size, in_axes=None)((*self.hidden_dims, self.latent_dim), activate_final=False, layer_norm=self.layer_norm)
            self.psi = ensemblize(mlp_module, self.ensemble_size, in_axes=0)((*self.hidden_dims, self.latent_dim), activate_final=False, layer_norm=self.layer_norm)
            if self.pred_both:
                self.psi_b = ensemblize(mlp_module, self.ensemble_size, in_axes=0)((*self.hidden_dims, self.latent_dim), activate_final=False, layer_norm=self.layer_norm)
        else:
            self.phi = mlp_module((*self.hidden_dims, self.latent_dim), activate_final=False, layer_norm=self.layer_norm)
            self.psi = mlp_module((*self.hidden_dims, self.latent_dim), activate_final=False, layer_norm=self.layer_norm)
            if self.pred_both:
                self.psi_b = mlp_module((*self.hidden_dims, self.latent_dim), activate_final=False, layer_norm=self.layer_norm)
                    
    def l2_normalize(self, x):
        eps = 1e-3
        return x / (jnp.linalg.norm(x, ord=2, axis=-1, keepdims=True) + eps)
    ...
    def __call__(
        ...
        if self.state_encoder is not None:
            target = self.state_encoder(target)
            pred_from = self.goal_encoder(pred_from)

        phi = self.phi(target)
        phi_goals = self.phi(pred_from)
        extra['phi_goal'] = phi_goals
        if psi_obs:
            extra['psi_obs'] = self.psi(phi)

        if self.normalize_phi:
            phi_goals = self.l2_normalize(phi_goals)
            phi = self.l2_normalize(phi)
            if psi_obs:
                extra['psi_obs'] = self.l2_normalize(psi_obs)
        
        if self.action_forward and actions is None:
            if use_both:
                return phi, None, None, extra    
            else:
                return phi, None, extra

        f_input = phi_goals
        # action-conditioned prediction
        if self.action_forward:
            actions = jnp.expand_dims(actions, axis=0)
            actions = jnp.repeat(actions, self.ensemble_size, axis=0)
            f_input = jnp.concatenate([phi_goals, actions], axis=-1)

        
        if use_both:
            psi_f = self.psi(f_input)
            psi_b = self.psi_b(phi_goals)
            return phi, psi_f, psi_b, extra
        elif use_backwards:
            psi_b = self.psi_b(phi_goals)
            return phi, psi_b, extra
        else:
            psi_f = self.psi(f_input)
            return phi, psi_f, extra
...
class Encoder(nn.Module):
    ...
    def setup(self) -> None:
        mlp_module = MLP
        if self.identity_phi:
            self.phi = Identity()
            if self.image_encoder is None:
                # add dummy parameter
                self.dummy_param = self.param('dummy', nn.initializers.zeros, (1,))
        else:
            if self.ensemble:
                self.phi = ensemblize(mlp_module, self.ensemble_size, in_axes=None)((*self.hidden_dims, self.latent_dim), activate_final=False, layer_norm=self.layer_norm)
            else:
    ...
    def __call__(
        ...
        if self.image_encoder is not None:
            observations = self.image_encoder(observations)
        representation = self.phi(observations)
...
class LatentPredictor(nn.Module):
    ...
    def setup(self) -> None:
        mlp_module = MLP
        self.psi_b = None
        psi_out = self.latent_dim

        if self.ensemble:
            self.psi = ensemblize(mlp_module, self.ensemble_size, in_axes=0)((*self.hidden_dims, psi_out), activate_final=False, layer_norm=self.layer_norm)
            if self.pred_both:
                self.psi_b = ensemblize(mlp_module, self.ensemble_size, in_axes=0)((*self.hidden_dims, psi_out), activate_final=False, layer_norm=self.layer_norm)
        else:
            self.psi = mlp_module((*self.hidden_dims, psi_out), activate_final=False, layer_norm=self.layer_norm)
            if self.pred_both:
                self.psi_b = mlp_module((*self.hidden_dims, psi_out), activate_final=False, layer_norm=self.layer_norm)
    ...
    def __call__(
        ...
        Args:
            ...
            actions: Actions (optional).
            use_backwards: Whether to use backwards prediction.
            use_both: Whether to use both forwards and backwards prediction.
        """

        f_input = representations
        b_input = representations
        if self.action_forward:
            if self.ensemble:
                actions = jnp.expand_dims(actions, axis=0)
                actions = jnp.repeat(actions, self.ensemble_size, axis=0)
            f_input = jnp.concatenate([representations, actions], axis=-1)
        if use_both:
            forward_pred = self.psi(f_input)
            back_pred = self.psi_b(b_input)
            return forward_pred, back_pred
        if use_backwards:
            back_pred = self.psi_b(b_input)
            return back_pred
        else:
